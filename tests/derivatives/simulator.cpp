...
#include "simple/core/simulator-derivatives.hpp"
  ...
  Eigen::MatrixXd & dlam_dtau_fd)
{
  ...
  sim_fd.reset();
  sim_fd.step<ADMM>(q, v, tau, dt);
  Eigen::VectorXd lam = sim_fd.workspace.constraint_problem().point_contact_constraint_forces();
  double delta = 1e-6;
  // finite differences on q
  sim_fd.reset();
  for (int i = 0; i < sim_fd.model().nv; i++)
  {
    ...
    qplus = pinocchio::integrate(sim_fd.model(), q, dq);
    sim_fd.step<ADMM>(qplus, v, tau, dt);
    dlam_dq_fd.col(colid) = (sim_fd.workspace.constraint_problem().point_contact_constraint_forces() - lam) / delta;
    sim_fd.reset();
  }
  // finite differences on v
  for (int i = 0; i < sim_fd.model().nv; i++)
  {
    ...
    vplus = v + dv;
    sim_fd.step<ADMM>(q, vplus, tau, dt);
    dlam_dv_fd.col(colid) = (sim_fd.workspace.constraint_problem().point_contact_constraint_forces() - lam) / delta;
  ...
  for (int i = 0; i < sim_fd.model().nv; i++)
  {
    ...
    tauplus = tau + dtau;
    sim_fd.step<ADMM>(q, v, tauplus, dt);
    dlam_dtau_fd.col(colid) = (sim_fd.workspace.constraint_problem().point_contact_constraint_forces() - lam) / delta;
    sim_fd.reset();
  }
}
...
BOOST_AUTO_TEST_CASE(simulator_derivatives_ball_plane)
{
  ...
  hpp::fcl::CollisionGeometryPtr_t sphere_ptr(new hpp::fcl::Sphere(r));
  ...
  Simulator sim(model, geom_model);
  sim.settings.constraint_solvers_settings.admm_settings.max_iterations = 100;
  sim.settings.constraint_solvers_settings.admm_settings.absolute_feasibility_tol = 1e-15;
  sim.settings.constraint_solvers_settings.admm_settings.relative_feasibility_tol = 1e-15;
  sim.settings.constraint_solvers_settings.admm_settings.absolute_complementarity_tol = 1e-15;
  sim.settings.constraint_solvers_settings.admm_settings.relative_complementarity_tol = 1e-15;
  ...
  Simulator sim_fd(model, geom_model);
  sim_fd.settings.constraint_solvers_settings.admm_settings.max_iterations = 100;
  sim_fd.settings.constraint_solvers_settings.admm_settings.absolute_feasibility_tol = 1e-15;
  sim_fd.settings.constraint_solvers_settings.admm_settings.absolute_complementarity_tol = 1e-15;
  sim_fd.settings.constraint_solvers_settings.admm_settings.relative_feasibility_tol = 1e-15;
  sim_fd.settings.constraint_solvers_settings.admm_settings.relative_complementarity_tol = 1e-15;
...
BOOST_AUTO_TEST_CASE(simulator_derivatives_ball_plane_gd)
{
  ...
  sim.step<ADMM>(q, v, tau, dt);
  Eigen::VectorXd contact_forces = sim.workspace.constraint_problem().point_contact_constraint_forces();
  ...
  for (int iters = 0; iters < max_gd_iters; ++iters)
  {
    ...
    sim.step<ADMM>(q, v, tau, dt);
    contact_forces = sim.workspace.constraint_problem().point_contact_constraint_forces();
...
BOOST_AUTO_TEST_CASE(simulator_derivatives_ball_plane_with_compliance)
{
  ...
  Simulator sim(model, geom_model);
  sim.settings.constraint_solvers_settings.admm_settings.max_iterations = 100;
  sim.settings.constraint_solvers_settings.admm_settings.absolute_feasibility_tol = 1e-12;
  sim.settings.constraint_solvers_settings.admm_settings.absolute_complementarity_tol = 1e-12;
  sim.settings.constraint_solvers_settings.admm_settings.relative_feasibility_tol = 1e-12;
  sim.settings.constraint_solvers_settings.admm_settings.relative_complementarity_tol = 1e-12;
  ...
  Simulator sim_fd(model, geom_model);
  sim_fd.settings.constraint_solvers_settings.admm_settings.max_iterations = 100;
  sim_fd.settings.constraint_solvers_settings.admm_settings.absolute_feasibility_tol = 1e-12;
  sim_fd.settings.constraint_solvers_settings.admm_settings.absolute_complementarity_tol = 1e-12;
  sim_fd.settings.constraint_solvers_settings.admm_settings.relative_feasibility_tol = 1e-12;
  sim_fd.settings.constraint_solvers_settings.admm_settings.relative_complementarity_tol = 1e-12;
...
BOOST_AUTO_TEST_CASE(simulator_derivatives_balls_plane)
{
  ...
  for (std::size_t i = 0; i < N_balls; ++i)
  {
    ...
    hpp::fcl::CollisionGeometryPtr_t sphere_ptr(new hpp::fcl::Sphere(r / 10));
...
BOOST_AUTO_TEST_CASE(simulator_derivatives_colliding_balls)
{
  ...
  // std::cout << "qnew   " << qnew << std::endl;
  // std::cout << "vnew   " << vnew << std::endl;
  // std::cout << "contact forces" << sim.workspace.constraint_problem().point_contact_constraint_forces() << std::endl;
