    ...
    Eigen::MatrixXd & dlam_dtau_fd)
{
    ...
    for (int i = 0; i < sim_fd.model().nv; i++)
    {
        ...
        // std::cout << simplex::logging::DEBUG << "FD:" << std::endl;
        // sim_fd.workspace.constraint_problem().collectActiveSet();
        // for (size_t i = 0; i < sim_fd.workspace.constraint_problem().contact_modes.size(); ++i)
        // {
        //     ConstraintsProblemDerivatives::ContactMode cat = sim_fd.workspace.constraint_problem().contact_modes[i];
        //     switch (cat)
        //     {
        //     case ConstraintsProblemDerivatives::ContactMode::BREAKING:
        //         std::cout << "contact mode " << i << "  BREAKING" << std::endl;
        //         break;
        //     case ConstraintsProblemDerivatives::ContactMode::SLIDING:
        //         std::cout << "contact mode " << i << "  SLIDING" << std::endl;
        //         break;
        //     case ConstraintsProblemDerivatives::ContactMode::STICKING:
        //         std::cout << "contact mode " << i << "  STICKING" << std::endl;
        //     }
        // }
...
BOOST_AUTO_TEST_CASE(simulator_derivatives_ball_plane_with_compliance)
{
    ...
    //           << "Size of the constraints problem: " << sim.workspace.constraint_problem().constraints_problem_size() << std::endl;
    // std::cout << simplex::logging::DEBUG << "Size of the frictional point constraints problem: "
    //           << sim.workspace.constraint_problem().frictional_point_constraints_size() << std::endl;
    ...
    std::cout << simplex::logging::DEBUG << "simulator_derivatives_ball_plane_with_compliance: Testing sticking mode" << std::endl;
    // std::cout << simplex::logging::DEBUG << "Analysis:" << std::endl;
    // for (size_t i = 0; i < sim.workspace.constraint_problem().contact_modes.size(); ++i)
    // {
    //     ConstraintsProblemDerivatives::ContactMode cat = sim.workspace.constraint_problem().contact_modes[i];
    //     switch (cat)
    //     {
    //     case ConstraintsProblemDerivatives::ContactMode::BREAKING:
    //         std::cout << "contact mode " << i << "  BREAKING" << std::endl;
    ...
    // sanity check
    std::cout << simplex::logging::DEBUG << "dlam_dtheta: " << dsim.contact_solver_derivatives.dlam_dtheta() << std::endl;
    ...
    std::cout << simplex::logging::DEBUG << "simulator_derivatives_ball_plane_with_compliance: Testing sliding mode" << std::endl;
    // std::cout << simplex::logging::DEBUG << "Analysis:" << std::endl;
    // for (size_t i = 0; i < sim.workspace.constraint_problem().contact_modes.size(); ++i)
    // {
    //     ConstraintsProblemDerivatives::ContactMode cat = sim.workspace.constraint_problem().contact_modes[i];
