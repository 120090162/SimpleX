...
BOOST_AUTO_TEST_CASE(simulatorx_instance_step_with_friction_on_joints)
{
    ModelHandle model(new pin::Model());
...
BOOST_AUTO_TEST_CASE(simulatorx_instance_step_with_limits_on_joints_for_manipulator)
{
    ModelHandle model(new pin::Model());
    pin::buildModels::manipulator(*model);
...

    {
        ...
        sim.step<ADMM>(q, v, tau, dt);
        EIGEN_VECTOR_IS_APPROX(sim.state.vnew, VectorXd::Zero(model->nv), 1e-6);
        // std::cout << "Iteration count with active limits: " << sim.workspace.constraint_solvers.admm_solver.getIterationCount()
        //           << std::endl;
        INDEX_EQUALITY_CHECK(sim.workspace.constraint_solvers.admm_solver.getIterationCount(), 0);
    }
}
