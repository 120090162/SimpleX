#ifndef __simplex_solver_admm_solver_hxx__
#define __simplex_solver_admm_solver_hxx__
...
namespace simplex
{
    ...
    : pinocchio::visitors::ConstraintUnaryVisitorBase<ZeroInitialGuessMaxConstraintViolationVisitor<DriftVectorLike, Scalar>>
    {
        ...
        static void
        algo(const pinocchio::ConstraintModelBase<ConstraintModel> & cmodel, const DriftVectorLike & drift, Scalar & max_violation)
        {
        ...
        static void algo_impl(
            const pinocchio::CoulombFrictionConeTpl<Scalar> & set, const Eigen::MatrixBase<VectorLike> & drift, Scalar & max_violation)
        {
        ...
        template<typename VectorLike>
        static void
        algo_impl(const pinocchio::UnboundedSetTpl<Scalar> & set, const Eigen::MatrixBase<VectorLike> & drift, Scalar & max_violation)
        {
            PINOCCHIO_UNUSED_VARIABLE(set);
        ...
        template<typename VectorLike>
        static void algo_impl(const pinocchio::BoxSetTpl<Scalar> & set, const Eigen::MatrixBase<VectorLike> & drift, Scalar & max_violation)
        {
            PINOCCHIO_UNUSED_VARIABLE(set);
            const Scalar violation = drift.template lpNorm<Eigen::Infinity>();
            if (violation > max_violation)
        ...
        template<typename VectorLike>
        static void algo_impl(
            const pinocchio::JointLimitConstraintConeTpl<Scalar> & set, const Eigen::MatrixBase<VectorLike> & drift, Scalar & max_violation)
        {
            Scalar negative_orthant_violation = 0;
            if (set.getNegativeOrthant().size() > 0)
            {
                negative_orthant_violation = pinocchio::math::max(Scalar(0), drift.head(set.getNegativeOrthant().size()).maxCoeff());
            }
            Scalar positive_orthant_violation = 0;
            if (set.getPositiveOrthant().size() > 0)
            {
                positive_orthant_violation = -pinocchio::math::min(Scalar(0), drift.tail(set.getPositiveOrthant().size()).minCoeff());
            }
            const Scalar violation = pinocchio::math::max(negative_orthant_violation, positive_orthant_violation);
            if (violation > max_violation)
            {
                max_violation = violation;
            }
        ...
        static void algo_impl(const ConstraintSet & set, const Eigen::MatrixBase<VectorLike> & drift, Scalar & max_violation)
        {
            // do nothing
            PINOCCHIO_UNUSED_VARIABLE(set);
        ...
        template<typename ConstraintModel>
        static void
        run(const pinocchio::ConstraintModelBase<ConstraintModel> & cmodel, const DriftVectorLike & drift, Scalar & max_violation)
        {
        ...
        static void
        run(const pinocchio::ConstraintModelTpl<Scalar, Options, ConstraintCollectionTpl> & cmodel,
    ...
    template<template<typename T> class Holder, typename ConstraintModel, typename ConstraintModelAllocator, typename VectorLikeIn>
    ...
    bool ADMMContactSolverTpl<_Scalar>::solve(
        pinocchio::DelassusOperatorBase<DelassusDerived> & _delassus,
        ...
        const boost::optional<Scalar> rho_init,
        const boost::optional<Scalar> mu_prox_init,
...

    {
        ...
        typedef pinocchio::DelassusOperatorPreconditionedTpl<DelassusDerived, DiagonalPreconditioner> DelassusOperatorPreconditioned;
        DelassusDerived & delassus = _delassus.derived();

        Scalar mu_prox = this->mu_prox;
        if (mu_prox_init)
            mu_prox = *mu_prox_init;
        ...
        // the pre-conditioner.
        pinocchio::internal::getTimeScalingFromAccelerationToConstraints(constraint_models, dt, time_scaling_acc_to_constraints);
        pinocchio::internal::getTimeScalingFromConstraintsToPosition(time_scaling_acc_to_constraints, dt, time_scaling_constraints_to_pos);
        gs = g.array() / time_scaling_acc_to_constraints.array();
        const Scalar g_pos_norm_inf = (g.cwiseProduct(time_scaling_constraints_to_pos)).template lpNorm<Eigen::Infinity>();
        ...
        if (preconditioner)
        {
            ...
            PINOCCHIO_CHECK_INPUT_ARGUMENT(
                preconditioner_.getDiagonal().minCoeff() > Scalar(0), "Preconditioner should be a strictly positive vector.");
        ...
        // Init y
        pinocchio::internal::computeConeProjection(constraint_models, x_, y_);
        ...
        if (solve_ncp)
        {
            pinocchio::internal::computeDeSaxeCorrection(constraint_models, z_, s_);
        ...
        rhs = z_.array() * time_scaling_acc_to_constraints.array(); // back to constraint formulation level
        complementarity = pinocchio::internal::computeConicComplementarity(constraint_models, rhs, y_);

        // dual feasibility is computed in "position" on the z_ variable (and not on z_bar_).
        dual_feasibility_vector = rhs;
        pinocchio::internal::computeDualConeProjection(constraint_models, rhs, rhs);
        dual_feasibility_vector -= rhs;
        dual_feasibility = dual_feasibility_vector.template lpNorm<Eigen::Infinity>();
        const Scalar absolute_residual_warm_start = pinocchio::math::max(complementarity, dual_feasibility);

        dual_feasibility_vector.array() *= time_scaling_constraints_to_pos.array();
        dual_feasibility = dual_feasibility_vector.template lpNorm<Eigen::Infinity>();
        this->absolute_residual = pinocchio::math::max(complementarity, dual_feasibility);
        ...
        const Scalar absolute_residual_zero_guess = computeZeroInitialGuessMaxConstraintViolation(constraint_models, g);
        ...
        { // If true, this means that the zero value initial guess leads a better feasibility in the
            ...
            {
                {
                    PINOCCHIO_TRACY_ZONE_SCOPED_N("ADMMContactSolverTpl::solve - second computeDeSaxeCorrection");
                    pinocchio::internal::computeDeSaxeCorrection(constraint_models, z_, s_);
                }
                z_ += s_; // Add De Saxé shift
            }
        ...
        { // the initial guess is not solution of the problem so we run the ADMM algorithm
            ...
            case (ADMMUpdateRule::SPECTRAL): {
                if (this->problem_size > 1)
                {
                    ...
                    //          {
                    //            assert(
                    //              pinocchio::math::fabs((true_m - m) / pinocchio::math::max(true_m, m)) < 0.01
                    //              && "true_m and m are too far apart.");
                    //          }
                    assert(
                        pinocchio::math::fabs((true_L - L) / pinocchio::math::max(true_L, L)) < 0.01 && "true_L and L are too far apart.");
#endif // NDEBUG
                }
                else
                {
                ...
                admm_update_rule_container.spectral_rule =
                    ADMMSpectralUpdateRule(ratio_primal_dual, L, m, rho_power_factor, rho_update_ratio);
                if (rho_init)
                    rho = *rho_init;
                else
                {
                    Scalar rho_target = ADMMSpectralUpdateRule::computeRho(L, m, rho_power);
                    rho = (1 - rho_momentum) * rho_target + rho_momentum * this->rho;
                }
                break;
            }
            case (ADMMUpdateRule::LINEAR):
                admm_update_rule_container.linear_rule =
                    ADMMLinearUpdateRule(ratio_primal_dual, linear_update_rule_factor, rho_update_ratio);
                if (rho_init)
                    rho = *rho_init;
                else
            ...
            // clamp the rho
            rho = pinocchio::math::max(1e-8, rho);
            ...
            VectorXs z_bar_velocity = VectorXs::Zero(this->problem_size);
            ...
            for (; it <= Base::max_it; ++it)
            {
                ...
                {
                    // s-update
                    pinocchio::internal::computeDeSaxeCorrection(constraint_models, z_bar_, s_bar_);
                }
                ...
                // y-update
                rhs -= z_bar_ / (tau * rho);
                pinocchio::internal::computeScaledConeProjection(constraint_models, rhs, preconditioner_.getDiagonal(), y_bar_);

                // z-update
                // z_bar_ -= (tau * rho) * (x_bar_ - y_bar_);
                {
                    const VectorXs z_step = -(tau * rho) * (x_bar_ - y_bar_);
                    z_bar_velocity = dual_momentum * z_bar_velocity + z_step;
                    z_bar_ += z_bar_velocity;
                }
...

                {
                    ...
                    dx_bar_norm = dx_bar.template lpNorm<Eigen::Infinity>(); // check relative progress on x_bar
...

                {
                    ...
                    dy_bar_norm = dy_bar.template lpNorm<Eigen::Infinity>(); // check relative progress on y_bar
...

                {
                    ...
                    dz_bar_norm = dz_bar.template lpNorm<Eigen::Infinity>(); // check relative progress on z_bar
                }
                ...
                const Scalar dual_feasibility_admm = dual_feasibility_vector.template lpNorm<Eigen::Infinity>();
                dual_feasibility_vector.array() *= time_scaling_acc_to_constraints.array();
                const Scalar dual_feasibility_constraint = dual_feasibility_vector.template lpNorm<Eigen::Infinity>();
                dual_feasibility_vector.array() *= time_scaling_constraints_to_pos.array();
                dual_feasibility = dual_feasibility_vector.template lpNorm<Eigen::Infinity>();
                ...
                if (stat_record)
                {
                    ...
                    if (solve_ncp)
                    {
                        pinocchio::internal::computeDeSaxeCorrection(constraint_models, tmp, rhs);
                        tmp += rhs;
                    }

                    tmp.array() *= time_scaling_acc_to_constraints.array(); // back to constraint formulation level
                    rhs = tmp;
                    pinocchio::internal::computeDualConeProjection(constraint_models, rhs, rhs);
                    tmp -= rhs;
                ...
                if (pinocchio::check_expression_if_real<Scalar, false>(complementarity <= this->absolute_precision)
                    && pinocchio::check_expression_if_real<Scalar, false>(
                        dual_feasibility
                        <= this->absolute_precision + this->relative_precision * pinocchio::math::max(g_pos_norm_inf, z_norm_inf))
                    && pinocchio::check_expression_if_real<Scalar, false>(
                        primal_feasibility
                        <= this->absolute_precision + this->relative_precision * pinocchio::math::max(x_norm_inf, y_norm_inf)))
                ...
                if (pinocchio::check_expression_if_real<Scalar, false>(
                        dx_bar_norm <= this->relative_precision * pinocchio::math::max(x_bar_norm_inf, x_bar_previous_norm_inf))
                    && pinocchio::check_expression_if_real<Scalar, false>(
                        dy_bar_norm <= this->relative_precision * pinocchio::math::max(y_bar_norm_inf, y_bar_previous_norm_inf))
                    && pinocchio::check_expression_if_real<Scalar, false>(
                        dz_bar_norm <= this->relative_precision * pinocchio::math::max(z_bar_norm_inf, z_bar_previous_norm_inf)))
                ...
                if (cholesky_update_count < max_delassus_decomposition_updates && (it % rho_min_update_frequency) == 0)
                {
                    ...
                    case (ADMMUpdateRule::SPECTRAL):
                        update_delassus_factorization =
                            admm_update_rule_container.spectral_rule.eval(primal_feasibility, dual_feasibility_constraint, rho);
                        break;
                    case (ADMMUpdateRule::LINEAR):
                        update_delassus_factorization =
                            admm_update_rule_container.linear_rule.eval(primal_feasibility, dual_feasibility_constraint, rho);
                        break;
                    case (ADMMUpdateRule::CONSTANT):
                        break;
                    }
            ...
            this->relative_residual = pinocchio::math::max(
                dx_bar_norm / pinocchio::math::max(x_bar_norm_inf, x_bar_previous_norm_inf),
                dy_bar_norm / pinocchio::math::max(y_bar_norm_inf, y_bar_previous_norm_inf));
            this->relative_residual =
                pinocchio::math::max(this->relative_residual, dz_bar_norm / pinocchio::math::max(z_bar_norm_inf, z_bar_previous_norm_inf));
            this->absolute_residual = pinocchio::math::max(primal_feasibility, pinocchio::math::max(complementarity, dual_feasibility));
...
#endif // ifndef __simplex_solver_admm_solver_hxx__
