#ifndef __simplex_core_constraints_problem_derivatives_hxx__
#define __simplex_core_constraints_problem_derivatives_hxx__
...
namespace simplex
{
    ...
    void ConstraintsProblemDerivativesTpl<S, O, JointCollectionTpl>::clear()
    {
        // Clearing contacts info
        this->pairs_in_collision.clear();
        this->contact_id_to_collision_pair.clear();
        point_contact_constraint_placements.clear();
        this->m_num_contacts = 0;

        // Clearing vectors of references to constraint models and constraint datas.
        // Note: this needs to be done regardless because the constraint models might have changed.
    ...
    // --------------------------------------------------------------------------
    template<typename S, int O, template<typename, int> class JointCollectionTpl>
    void ConstraintsProblemDerivativesTpl<S, O, JointCollectionTpl>::allocate()
    {
        SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblemDerivatives::allocate");

        //
        // We first allocate memory for the constraints by setting their storage.
        //

        // -- Joint friction
        // Identify joints that have non-zero dry friction limits.
        // We collect their IDs and track their indices for the concatenated friction vector.
        ...
        for (std::size_t i = 1; i < (std::size_t)this->model().njoints; ++i)
        {
            ...
            // Check if upper or lower dry friction limits are non-zero for this joint
            if (!(this->model().upperDryFrictionLimit.segment(joint.idx_v(), joint.nv()).isZero())
                || !(this->model().lowerDryFrictionLimit.segment(joint.idx_v(), joint.nv()).isZero()))
            {
                // Save the starting index within the friction constraint vector
                dry_friction_joint_idx_v.push_back(size_dry_friction);
                // Save the joint's original ID
                active_friction_joints_ids.push_back(joint.id());
                // Accumulate the total degrees of freedom affected by friction
                size_dry_friction += joint.nv();
            }
        }
        // now we can build the holder for the frictional joint constraint
        joint_friction_lower_limit = VectorXs::Zero(size_dry_friction);
        joint_friction_upper_limit = VectorXs::Zero(size_dry_friction);
        for (std::size_t i = 0; i < active_friction_joints_ids.size(); ++i)
        {
            ...
            joint_friction_lower_limit.segment(dry_friction_joint_idx_v[i], joint.nv()) =
                this->model().lowerDryFrictionLimit.segment(joint.idx_v(), joint.nv());
            joint_friction_upper_limit.segment(dry_friction_joint_idx_v[i], joint.nv()) =
        ...
        fjcm.set() = BoxSet(joint_friction_lower_limit, joint_friction_upper_limit);
        ...
        // -- Frictional contacts
        // For now, it's assumed that there is only one `coal::Contact` per collision pair, meaning
        ...
        // pair (i.e. for two non-convex objects), we will have to recompute `max_num_contacts_`, by
        ...
        point_contact_constraint_placements.reserve(max_num_contacts);
        ...
        for (::pinocchio::PairIndex col_pair_id = 0; col_pair_id < this->geom_model().collisionPairs.size(); ++col_pair_id)
        {
            ...
            for (ContactIndex contact_id = 0; contact_id < this->m_max_num_contact_per_collision_pair; ++contact_id)
            {
                ...
                fpcm.name = "FrictionalPointConstraintModel_" + std::to_string(geom_id1) + "_" + std::to_string(geom_id2);
        ...
        const int max_constraint_problem_size = this->joint_friction_constraint_size()    //
                                                + this->bilateral_constraints_size()      //
                                                + this->weld_constraints_size()           //
                                                + this->joint_limit_constraint_max_size() //
                                                + frictional_contact_max_size;            //

        // reserve based on current estimate of max constraint problem size.
        // note: in any case, `update` calls `resize`.
        storage.reserve(max_constraint_problem_size);
    ...
    // --------------------------------------------------------------------------
    template<typename S, int O, template<typename, int> class JointCollectionTpl>
    void ConstraintsProblemDerivativesTpl<S, O, JointCollectionTpl>::update(const Scalar dt)
    {
        SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblemDerivatives::update");

        clear();
        assert(this->m_num_contacts == 0 && "The number of contacts should be 0.");
        assert(this->geom_data().collisionResults.size() == this->geom_model().collisionPairs.size());
        assert(this->checkConstraintsConsistencyWithGeometryModel());

        //
        // Fill vectors of reference wrappers for constraint models and constraint datas.
        // This needs to be done because the reference on constraints might have changed.
        // Call calc at the same time to update the computations in the constraint datas.
        ...
        {
            ConstraintModel & constraint_model = this->joint_friction_constraint_model;
            FrictionalJointConstraintModel * fjcm = boost::get<FrictionalJointConstraintModel>(&constraint_model);
            assert(fjcm != nullptr);
            if (fjcm != nullptr)
            {
                fjcm->set().lb().noalias() = joint_friction_lower_limit * dt;
                fjcm->set().ub().noalias() = joint_friction_upper_limit * dt;
            }
            ConstraintData & constraint_data = this->joint_friction_constraint_data;
            constraint_model.calc(this->model(), this->data(), constraint_data);

            if (constraint_model.activeSize() > 0)
            {
        ...
        {
            assert(this->bilateral_point_constraint_models.size() == this->bilateral_point_constraint_datas.size());
            for (std::size_t i = 0; i < this->bilateral_point_constraint_models.size(); ++i)
            {
                this->bilateral_point_constraint_models[i].calc(this->model(), this->data(), this->bilateral_point_constraint_datas[i]);
        ...
        // Weld constraints
        {
            assert(this->weld_constraint_models.size() == this->weld_constraint_datas.size());
            for (std::size_t i = 0; i < this->weld_constraint_models.size(); ++i)
            {
                this->weld_constraint_models[i].calc(this->model(), this->data(), this->weld_constraint_datas[i]);
        ...
        // Process joint limit constraints to check which ones are active
        {
            ...
            // note: `cmodel.calc(...)` automatically calls `cmodel.resize(...)` for joint limits.
            constraint_model.calc(this->model(), this->data(), constraint_data);
            if (constraint_model.activeSize() > 0)
            {
                this->constraint_models.emplace_back(constraint_model);
        ...
        //
        // Process contacts found by collision detection and transform them into contact constraints.
        ...
        for (GeomIndex col_pair_id = 0; col_pair_id < this->geom_model().collisionPairs.size(); col_pair_id++)
        {
            ...
            const coal::CollisionResult & col_res = this->geom_data().collisionResults[col_pair_id];
            const coal::ContactPatchResult & patch_res = this->geom_data().contactPatchResults[col_pair_id];
            ...
            if (col_res.isCollision() && patch_res.numContactPatches() > 0)
            {
                ...
                const coal::ContactPatch & patch = patch_res.getContactPatch(0);
                ...
                for (ContactIndex contact_id = 0; contact_id < num_contacts_colpair; ++contact_id)
                {
                    ...
                    point_contact_constraint_placements.emplace_back(oMc);
                    //
                    SE3 & oMc1 = oMc;
                    ...
                    assert(
                        std::abs(
                            (fpcdata->constraint_position_error - Vector3s(0, 0, patch.penetration_depth - col_req.security_margin)).norm())
        ...
        // Resize eigen storages and initialize constraint dynamics quantities to zero.
        // This is useful in case constraints are constructed but not solved.
        // In this case, the constraint forces/velocities are not polluted by previous results.
        storage.resize(this->constraints_problem_size());
        g.setZero();
        preconditioner.setZero();
        constraint_forces.setZero();
        constraint_velocities.setZero();
        time_scaling_acc_to_constraints.setZero();
    ...
    void ConstraintsProblemDerivativesTpl<S, O, JointCollectionTpl>::build(
        const Eigen::MatrixBase<FreeVelocityVectorType> & vfree, const Eigen::MatrixBase<VelocityVectorType> & v, Scalar dt)
    {
        SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblemDerivatives::build");
        ...
        PINOCCHIO_CHECK_ARGUMENT_SIZE(
            this->constraint_models.size(), //
            this->constraint_datas.size(),  //
            "The number of constraint models and datas should be the same.");
        // debug
        assert(check());

        // Compute `G`, the Delassus operator
        switch (delassus_type)
        {
        case (DelassusType::DENSE):
        case (DelassusType::CHOLESKY): {
            {
                SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblemDerivatives::build - resize Delassus cholesky");
                this->constraint_cholesky_decomposition.resize(this->model(), this->constraint_models);
            }
            {
                SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblemDerivatives::build - compute Delassus cholesky");
                // TODO: inside ContactCholeskyDecomposition, do:
                //   - introduce `min_rho_value`. In `compute`, use `max(damping, min_rho_value)`
                //   - by default, m_damping = VectorXs::Zero. updateDamping will update this vector
                //   - deprecate resize
                //   - introduce update (resize done in update, read compliances from constraint and store it in m_compliance)
                //   - remove updateDamping in compute
                //   - call update then compute
                // TODO: inside DelassusCholeskyExpression, add the update and compute methods which call update and compute on the cholesky
                // decomposition -> this will align the rigid body delassus and the cholesky delassus
                const Scalar dummy_rho = Scalar(1e-8);
                this->constraint_cholesky_decomposition.compute(
                    this->model(), this->data(), this->constraint_models, this->constraint_datas, dummy_rho);
            }
            break;
        }
        ...
        {
            SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblemDerivatives::build - compute g");
            computeConstraintsDrift(vfree, v, dt);
        }

        preconditioner.setOnes();
    }

    // --------------------------------------------------------------------------
    template<typename S, int O, template<typename, int> class JointCollectionTpl>
    bool ConstraintsProblemDerivativesTpl<S, O, JointCollectionTpl>::check() const
    {
        ...
        // if NOT defined NDEBUG, perform extra checks
        assert(
            (storage.g_storage.storage().size() >= static_cast<Eigen::Index>(this->constraints_problem_size()))
            && "The size of g_ is too small to contain the contact problem.");
        assert(
    ...
    // --------------------------------------------------------------------------
    template<typename S, int O, template<typename, int> class JointCollectionTpl>
    void ConstraintsProblemDerivativesTpl<S, O, JointCollectionTpl>::collectActiveSet(Scalar epsilon)
    {
        ...
        {
            const Vector3s lambda_i = frictional_point_constraints_forces().template segment<3>(3 * contact_id);
            // TODO(quentinll) check if the compliance is taken into account in sigma_i. It should be.
            const Vector3s sigma_i = frictional_point_constraints_velocities().template segment<3>(3 * contact_id);
            if (lambda_i.norm() < epsilon) // breaking
            {
                this->breaking_contacts.push_back((ContactIndex)contact_id);
                this->contact_modes.push_back(Base::ContactMode::BREAKING);
            }
            else if (sigma_i.norm() < epsilon) // sticking
            {
                this->sticking_contacts.push_back((ContactIndex)contact_id);
                this->contact_modes.push_back(Base::ContactMode::STICKING);
            }
            else // sliding
            {
                this->sliding_contacts.push_back((ContactIndex)contact_id);
                this->contact_modes.push_back(Base::ContactMode::SLIDING);
            }
        }
    ...
    template<typename S, int O, template<typename, int> class JointCollectionTpl>
    template<typename FreeVelocityVectorType, typename VelocityVectorType>
    void ConstraintsProblemDerivativesTpl<S, O, JointCollectionTpl>::computeConstraintsDrift(
        const Eigen::MatrixBase<FreeVelocityVectorType> & vfree, const Eigen::MatrixBase<VelocityVectorType> & v, const Scalar dt)
    {
        ...
        for (std::size_t i = 0; i < this->constraint_models.size(); ++i)
        {
            ...
            // Compute constraint velocity warm-start = Jc * v
            auto constraint_velocity_warmstart = constraint_velocities.segment(cindex, cdim);
            cmodel.jacobianMatrixProduct(this->model(), this->data(), cdata, v, constraint_velocity_warmstart);
...
#endif // __simplex_core_constraints_problem_derivatives_hxx__