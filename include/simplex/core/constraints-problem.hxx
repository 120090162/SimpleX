#ifndef __simplex_core_constraints_problem_hxx__
#define __simplex_core_constraints_problem_hxx__
...
namespace simplex
{
	...
	ConstraintsProblemTpl<S, O, JointCollectionTpl>::ConstraintsProblemTpl(
		...
		: m_model(model_handle), m_data(data_handle), m_geom_model(geom_model_handle), m_geom_data(geom_data_handle), m_current_constraints_pb_id(0), m_max_num_contact_per_collision_pair(4), m_num_contacts(0), m_previous_num_contacts(0), m_previous_joint_limits_constraint_forces_size(0)
	{
	...
	{
		SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblem::allocate");
		...
		for (std::size_t i = 1; i < (std::size_t)this->model().njoints; ++i)
		{
			...
			if (
				!(this->model().upperDryFrictionLimit.segment(joint.idx_v(), joint.nv()).isZero()) || !(this->model().lowerDryFrictionLimit.segment(joint.idx_v(), joint.nv()).isZero()))
			{
	...
	{
		SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblem::update");
		...
		for (GeomIndex col_pair_id = 0; col_pair_id < this->geom_model().collisionPairs.size(); col_pair_id++)
		{
			...
			if (col_res.isCollision() && patch_res.numContactPatches() > 0)
			{
				...
				for (ContactIndex contact_id = 0; contact_id < num_contacts_colpair; ++contact_id)
				{
					...
					assert(
						std::abs((fpcdata->constraint_position_error - Vector3s(0, 0, patch.penetration_depth - col_req.security_margin)).norm()) < std::sqrt(Eigen::NumTraits<Scalar>::dummy_precision()));
#endif // NDEBUG
					...
					if (compute_warm_start && this->previous_colliding_collision_pairs[col_pair_id])
					{
						...
						warm_start_force.noalias() = cprevMc.rotation() * this->previous_frictional_point_constraints_forces().template segment<3>(
		...
		S dt)
	{
		...
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblem::build - resize Delassus cholesky");
			this->constraint_cholesky_decomposition.resize(this->model(), this->constraint_models);
		}

		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblem::build - compute Delassus cholesky");
		...
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("ConstraintsProblem::build - compute g");
	...
	bool ConstraintsProblemTpl<S, O, JointCollectionTpl>::check() const
	{
		...
		assert(
			(this->frictional_point_constraints_warmstart().size() == this->frictional_point_constraints_size()) && "The size of the contact velocity warm-start does not match the size of the contact "
																													"problem.");
		assert(
			(this->checkConstraintsConsistencyWithGeometryModel()) && "The constraints model and data vectors are not consistent with the geom_model and the "
																	  "geom_data.");
...
#endif // __simplex_core_constraints_problem_hxx__
