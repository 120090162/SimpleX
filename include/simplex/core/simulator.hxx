...
namespace simplex
{
      ...
      this->vfree = v + dt * ::pinocchio::aba(this->model(), this->data(), q, v, this->tau_total, this->ftotal, pinocchio::Convention::WORLD);
        ...
        if (this->constraints_problem().constraints_problem_size() > 0)
        {
...

            {
                ...
                this->anew =
                    ::pinocchio::aba(this->model(), this->data(), q, v, this->tau_total, this->ftotal, pinocchio::Convention::WORLD);
    ...
    if (this->measure_timings) { this->m_internal_timer.start(); }
        ...
        // --> Narrow Phase
        {
            ...
            for (std::size_t i = 0; i < this->m_collision_callback.pair_indexes.size(); ++i)
            {
                ...
                if (obj1.geometry->getNodeType() == coal::GEOM_PLANE || obj1.geometry->getNodeType() == coal::GEOM_HALFSPACE
    ...
    void SimulatorTpl<S, O, JointCollectionTpl>::resolveConstraints(const Eigen::MatrixBase<VelocityVectorType> & v, const Scalar dt)
    {
        ...
        assert(
            !this->constraints_problem().constraint_models.empty()
    ...
    namespace details
    {
        ...
        SimulatorConstraintSolverTpl<::pinocchio::ADMMContactSolverTpl, _Scalar, _Options, JointCollectionTpl>::setup(Simulator & simulator)
        {
            ...
            if (simulator.isReset() || simulator.constraint_solver_type != Simulator::ConstraintSolverType::ADMM
            ...
            Simulator & simulator, Scalar dt)
        {
            ...
            solver.solve(
                ...
                boost::make_optional((RefConstVectorXs)constraints_forces), boost::none, constraints_problem.is_ncp,
                settings.admm_update_rule, settings.stat_record);
        ...
        template<typename _Scalar, int _Options, template<typename, int> class JointCollectionTpl>
        void
        SimulatorConstraintSolverTpl<::pinocchio::PGSContactSolverTpl, _Scalar, _Options, JointCollectionTpl>::setup(Simulator & simulator)
        {
            ...
            const auto problem_size = static_cast<int>(simulator.constraints_problem().constraints_problem_size());
            if (simulator.isReset() || simulator.constraint_solver_type != Simulator::ConstraintSolverType::PGS
                || solver.getPrimalSolution().size() != problem_size)
            {
