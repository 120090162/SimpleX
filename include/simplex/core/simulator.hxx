#ifndef __simplex_core_simulator_hxx__
#define __simplex_core_simulator_hxx__
...
namespace simplex
{
	...
	SimulatorTpl<S, O, JointCollectionTpl>::SimulatorTpl(
		...
		: m_model(model_handle), m_data(data_handle), m_geom_model(geom_model_handle), m_geom_data(geom_data_handle), q(this->model().nq), v(this->model().nv), tau(this->model().nv), fext(static_cast<std::size_t>(this->model().njoints), Force::Zero()), dt(-1), qnew(this->model().nq), vfree(this->model().nv), vnew(this->model().nv), anew(this->model().nv), ftotal(static_cast<std::size_t>(this->model().njoints), Force::Zero()), tau_total(this->model().nv), tau_constraints(this->model().nv), constraint_solver_type(ConstraintSolverType::NONE), admm_constraint_solver(0) // only create it when needed
		  ,
		  ...
		  m_collision_callback(this->geom_model(), this->geom_data()), m_constraints_problem(new ConstraintsProblem(
																		   model_handle, data_handle, geom_model_handle, geom_data_handle, bilateral_constraint_models, weld_constraint_models)),
		  m_vnew_integration_tmp(this->model().nv), m_is_reset(true), m_step_timer(false), m_internal_timer(false)
	{
	...
	void SimulatorTpl<S, O, JointCollectionTpl>::initializeGeometryData()
	{
		...
		// Narrow phase setup
		for (coal::CollisionRequest &request : this->geom_data().collisionRequests)
		{
	...
	void SimulatorTpl<S, O, JointCollectionTpl>::step(
		...
		const Scalar dt_)
	{
		SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::step");
		// clang-format off
    if (this->measure_timings) { this->m_step_timer.start(); };
		...
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::step - compute crba");
			::pinocchio::crba(this->model(), this->data(), q, pinocchio::Convention::WORLD);
		}

		// Compute free acceleration of the system
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::step - compute vfree (first aba)");
			this->ftotal = fext;
			this->tau_total = tau;
			// clang-format off
      this->vfree = v + dt * ::pinocchio::aba(this->model(), this->data(), q, v, this->tau_total, this->ftotal, pinocchio::Convention::WORLD);
		...
		if (this->constraints_problem().constraints_problem_size() > 0)
		{
			...
			{
				SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::step - compute vnew (second aba)");
	...
	{
		SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::detectCollisions");
    ...
    if (this->measure_timings) { this->m_internal_timer.start(); }
		...
		for (coal::CollisionResult &col_res : this->geom_data().collisionResults)
		{
			col_res.clear();
		}
		...
		// --> Broad Phase
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::detectCollisions - Broad phase");
			::pinocchio::computeCollisions(*(this->m_broad_phase_manager), &this->m_collision_callback);
		}

		// --> Narrow Phase
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::detectCollisions - Narrow phase");
			for (std::size_t i = 0; i < this->m_collision_callback.pair_indexes.size(); ++i)
			{
				...
				if (
					obj1.geometry->getNodeType() == coal::GEOM_PLANE || obj1.geometry->getNodeType() == coal::GEOM_HALFSPACE || obj2.geometry->getNodeType() == coal::GEOM_PLANE || obj2.geometry->getNodeType() == coal::GEOM_HALFSPACE)
				{
					obb_overlap = true;
				}
				else
	...
	{
		SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::resolveConstraints");
    ...
    this->m_constraint_solver_timings = this->m_internal_timer.elapsed();
		...
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("Simulator::resolveConstraints - apply constraint forces");
	...
	bool SimulatorTpl<S, O, JointCollectionTpl>::check() const
	{
		...
		assert(
			this->checkCollisionPairs() && "The GeometryModel contains collision pairs between GeometryObjects attached to the same "
										   "joint.");
	...
	namespace details
	{
		...
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("SimulatorConstraintSolver<ADMM>::setup");
			...
			if (
				simulator.isReset() || simulator.constraint_solver_type != Simulator::ConstraintSolverType::ADMM || solver.getPrimalSolution().size() != problem_size)
			{
		...
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("SimulatorConstraintSolver<ADMM>::run");
		...
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("SimulatorConstraintSolver<PGS>::setup");
			...
			const auto problem_size = static_cast<int>(simulator.constraints_problem().constraints_problem_size());
			if (
				simulator.isReset() || simulator.constraint_solver_type != Simulator::ConstraintSolverType::PGS || solver.getPrimalSolution().size() != problem_size)
			{
		...
		void SimulatorConstraintSolverTpl<::pinocchio::PGSContactSolverTpl, _Scalar, _Options, JointCollectionTpl>::run(
			Simulator &simulator, Scalar dt)
		{
			SIMPLEX_TRACY_ZONE_SCOPED_N("SimulatorConstraintSolver<PGS>::run");
			ConstraintsProblem &constraints_problem = simulator.constraints_problem();
			PGSConstraintSolver &solver = simulator.pgs_constraint_solver;
			const PGSConstraintSolverSettings &settings = simulator.pgs_constraint_solver_settings;
...
#endif // __simplex_core_simulator_hxx__
