...
namespace simplex
{
	...
	public:
		...
		coal::CPUTimes getStepCPUTimes() const { return this->m_step_timings; }
		...
		coal::CPUTimes getConstraintSolverCPUTimes() const { return this->m_constraint_solver_timings; }

		/// \brief Get timings of the collision detection stage.
		coal::CPUTimes getCollisionDetectionCPUTimes() const { return this->m_collision_detection_timings; }
	...
	public:
		...
		SimulatorTpl(ModelHandle model_handle, DataHandle data_handle, GeometryModelHandle geom_model_handle, GeometryDataHandle geom_data_handle,
					 const BilateralPointConstraintModelVector &bilateral_constraint_models, const WeldConstraintModelVector &weld_constraint_models);
		...
		template <template <typename> class ConstraintSolver = ::pinocchio::ADMMContactSolverTpl, typename ConfigVectorType, typename VelocityVectorType, typename TorqueVectorType>
		void step(const Eigen::MatrixBase<ConfigVectorType> &q, const Eigen::MatrixBase<VelocityVectorType> &v,
				  const Eigen::MatrixBase<TorqueVectorType> &tau, Scalar dt);
		...
		**/
		template <template <typename> class ConstraintSolver = ::pinocchio::ADMMContactSolverTpl, typename ConfigVectorType, typename VelocityVectorType, typename TorqueVectorType, typename ForceDerived>
		void step(const Eigen::MatrixBase<ConfigVectorType> &q, const Eigen::MatrixBase<VelocityVectorType> &v, const Eigen::MatrixBase<TorqueVectorType> &tau,
				  const pinocchio::container::aligned_vector<ForceDerived> &fext, const Scalar dt);

		/// \brief Returns true if the simulator is in its reset state.
		bool isReset() const { return this->m_is_reset; }
		...
		const Model &model() const { return pinocchio::helper::get_ref(this->m_model); }

		/// \brief Returns a reference to the model
		Model &model() { return pinocchio::helper::get_ref(this->m_model); }

		/// \brief Returns a const reference to the data
		const Data &data() const { return pinocchio::helper::get_ref(this->m_data); }

		/// \brief Returns a reference to the data
		Data &data() { return pinocchio::helper::get_ref(this->m_data); }

		/// \brief Returns a const reference to the geometry model
		const pinocchio::GeometryModel &geom_model() const { return pinocchio::helper::get_ref(this->m_geom_model); }

		/// \brief Returns a reference to the geometry model
		pinocchio::GeometryModel &geom_model() { return pinocchio::helper::get_ref(this->m_geom_model); }

		/// \brief Returns a const reference to the geometry data
		const pinocchio::GeometryData &geom_data() const { return pinocchio::helper::get_ref(this->m_geom_data); }

		/// \brief Returns a reference to the geometry data
		pinocchio::GeometryData &geom_data() { return pinocchio::helper::get_ref(this->m_geom_data); }

		/// \brief Returns a const reference to the constraint problem
		const ConstraintsProblem &constraints_problem() const { return pinocchio::helper::get_ref(this->m_constraints_problem); }

		/// \brief Returns a reference to the constraint problem
		ConstraintsProblem &constraints_problem() { return pinocchio::helper::get_ref(this->m_constraints_problem); }

		/// \brief Returns a handle to the constraint problem
		ConstraintsProblemHandle getConstraintsProblemHandle() const { return this->m_constraints_problem; }
	...
	protected:
		...
		virtual void preambleResolveConstraints(const Scalar dt) { PINOCCHIO_UNUSED_VARIABLE(dt); }
