    ...
    if FLAGS.display:
        ...
        def get_callback(i: int):
            def _callback(t):
                ...
                vizer.viewer["/Cameras/default/position"].set_property("position", list(cps_[i]))
                vizer.viewer["/Cameras/default/lookAt"].set_property("position", list(rps_[i]))
        ...
        if FLAGS.record:
            ctx = vizer.create_video_ctx(dir_path/"logs/simulate/simulation.mp4", fps=fps)
        else:
            import contextlib
