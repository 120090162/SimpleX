...
// SPDX-License-Identifier: MIT
    ...
    // SPDX-License-Identifier: MIT
        ...
        #if !defined(FK_YAML_HAS_CXX_23) && !defined(FK_YAML_HAS_CXX_20) && !defined(FK_YAML_HAS_CXX_17) && !defined(FK_YAML_HAS_CXX_14)   \
...
namespace make_int_seq_impl
{
    ...
    struct generator
    {
        using type = typename merger<typename generator<T, Num / 2>::type, typename generator<T, Num - Num / 2>::type>::type;
    };
...
namespace detect
{
    ...
    : conjunction<
          is_detected<difference_type_t, remove_cvref_t<T>>,
          has_value_type<remove_cvref_t<T>>,
          is_detected<pointer_t, remove_cvref_t<T>>,
          is_detected<reference_t, remove_cvref_t<T>>,
...
template<
    template<typename, typename...> class SequenceType,
    template<typename, typename, typename...> class MappingType,
    typename BooleanType,
    typename IntegerType,
    typename FloatNumberType,
    typename StringType,
    template<typename, typename> class Converter>
struct is_basic_node_impl<basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType, Converter>>
: std::true_type
{
};
...
struct has_from_node<BasicNodeType, T, enable_if_t<negation<is_basic_node<T>>::value>>
{
    ...
    static constexpr bool value = is_detected_exact<void, from_node_function_t, converter, const BasicNodeType &, T &>::value;
};
...
struct is_node_compatible_type_impl<
    BasicNodeType,
    CompatibleType,
    enable_if_t<conjunction<is_complete_type<CompatibleType>, has_to_node<BasicNodeType, CompatibleType>>::value>> : std::true_type
{
};
...
private:
    ...
    static std::string generate_error_message(const char * msg, std::array<char16_t, 2> u16) noexcept
    {
        ...
        return detail::format(
            "invalid_encoding: %s in=[ 0x%04x, 0x%04x ]", msg, static_cast<uint16_t>(u16[0]), static_cast<uint16_t>(u16[1]));
    }
...
namespace utf8
{
    ...
    inline void from_utf16(std::array<char16_t, 2> utf16, std::array<uint8_t, 4> & utf8, uint32_t & consumed_size, uint32_t & encoded_size)
    {
    ...
    inline void from_utf32(const char32_t utf32, std::array<uint8_t, 4> & utf8, uint32_t & encoded_size)
    {
        ...
        {
            const uint32_t utf8_chunk =
                0xF0808080u | ((utf32 & 0x1C0000u) << 6) | ((utf32 & 0x03F000u) << 4) | ((utf32 & 0x0FC0u) << 2) | (utf32 & 0x3Fu);
            utf8[0] = static_cast<uint8_t>(utf8_chunk >> 24);
            utf8[1] = static_cast<uint8_t>(utf8_chunk >> 16);
            utf8[2] = static_cast<uint8_t>(utf8_chunk >> 8);
            utf8[3] = static_cast<uint8_t>(utf8_chunk);
...
class basic_str_view
{
    ...
    static_assert(std::is_trivial<CharT>::value && std::is_standard_layout<CharT>::value, "CharT must be a trivial, standard layout type.");
    static_assert(std::is_same<CharT, typename Traits::char_type>::value, "CharT & Traits::char_type must be the same type.");
...
public:
    ...
    template<
        ...
        enable_if_t<
            conjunction<
                negation<std::is_array<CharPtrT>>,
                std::is_pointer<CharPtrT>,
    ...
    template<
        ...
        enable_if_t<
            conjunction<
                ...
                std::is_base_of<std::random_access_iterator_tag, typename std::iterator_traits<ItrType>::iterator_category>>::value,
            int> = 0>
    ...
    int compare(basic_str_view sv) const noexcept
    {
        ...
        if (ret == 0)
        {
            ...
            const difference_type diff =
                m_len > sv.m_len ? m_len - sv.m_len : static_cast<difference_type>(-1) * static_cast<difference_type>(sv.m_len - m_len);
...
template<typename CharT, typename Traits>
inline std::basic_ostream<CharT, Traits> & operator<<(std::basic_ostream<CharT, Traits> & os, basic_str_view<CharT, Traits> sv)
{
...
public:
    ...
    lexical_token get_next_token()
    {
        ...
        case '>': {
            ...
            const lexical_token_t type =
                *m_token_begin_itr == '|' ? lexical_token_t::BLOCK_LITERAL_SCALAR : lexical_token_t::BLOCK_FOLDED_SCALAR;
            ...
            return {type, determine_block_scalar_content_range(base_indent, m_block_scalar_header.indent, m_block_scalar_header.indent)};
        }
        default:
            break;
        }
...
private:
    ...
    str_view determine_block_scalar_content_range(uint32_t base_indent, uint32_t indicated_indent, uint32_t & content_indent)
    {
        const str_view sv{m_token_begin_itr, m_end_itr};
        ...
        if FK_YAML_UNLIKELY (cur_itr == m_end_itr)
        {
            ...
            content_indent = indicated_indent == 0 ? std::max(cur_indent, max_leading_indent) : base_indent + indicated_indent;
            return sv;
        }
...
inline bool atob(CharItr begin, CharItr end, BoolType & boolean) noexcept
{
    ...
    {
        const bool is_true =
            (std::strncmp(p_begin, "true", 4) == 0) || (std::strncmp(p_begin, "True", 4) == 0) || (std::strncmp(p_begin, "TRUE", 4) == 0);
...
{
    static_assert(is_non_bool_integral<IntType>::value, "atoi_dec_unchecked() accepts non-boolean integral types as an output type");
...
{
    static_assert(is_non_bool_integral<IntType>::value, "atoi_dec_pos() accepts non-boolean integral types as an output type");
...
{
    static_assert(is_non_bool_integral<IntType>::value, "atoi_dec_neg() accepts non-boolean integral types as an output type");
...
{
    static_assert(is_non_bool_integral<IntType>::value, "atoi_oct() accepts non-boolean integral types as an output type");
...
{
    static_assert(is_non_bool_integral<IntType>::value, "atoi_hex() accepts non-boolean integral types as an output type");
...
inline bool atof(CharItr begin, CharItr end, FloatType & f) noexcept(noexcept(atof_impl(&*begin, &*begin, f)))
{
    ...
    if (*p_begin == '-' || *p_begin == '+')
    {
        if (len == 5)
        {
            ...
            const bool is_inf = (std::strncmp(p_from_second, ".inf", 4) == 0) || (std::strncmp(p_from_second, ".Inf", 4) == 0)
    ...
    {
        const bool is_inf =
            (std::strncmp(p_begin, ".inf", 4) == 0) || (std::strncmp(p_begin, ".Inf", 4) == 0) || (std::strncmp(p_begin, ".INF", 4) == 0);
        if (is_inf)
        {
        ...
        const bool is_nan =
            (std::strncmp(p_begin, ".nan", 4) == 0) || (std::strncmp(p_begin, ".NaN", 4) == 0) || (std::strncmp(p_begin, ".NAN", 4) == 0);
        if (is_nan)
        {
            set_nan(f);
            return true;
...
public:
    ...
    static node_type scan(const char * begin, const char * end) noexcept
    {
        ...
        case 4:
            ...
            case 'N':
                // no possible case of begin a number otherwise.
                return ((std::strncmp(p_begin + 1, "ull", 3) == 0) || (std::strncmp(p_begin + 1, "ULL", 3) == 0)) ? node_type::NULL_OBJECT
                                                                                                                  : node_type::STRING;
            case 't':
            ...
            case 'T':
                // no possible case of being a number otherwise.
                return ((std::strncmp(p_begin + 1, "rue", 3) == 0) || (std::strncmp(p_begin + 1, "RUE", 3) == 0)) ? node_type::BOOLEAN
                                                                                                                  : node_type::STRING;
            case '.': {
                const char * p_from_second = p_begin + 1;
        ...
        case 5:
            ...
            case 'F':
                // no possible case of being a number otherwise.
                return ((std::strncmp(p_begin + 1, "alse", 4) == 0) || (std::strncmp(p_begin + 1, "ALSE", 4) == 0)) ? node_type::BOOLEAN
                                                                                                                    : node_type::STRING;
            case '+':
            case '-':
                if (*(p_begin + 1) == '.')
                {
                    ...
                    const bool is_min_inf = (std::strncmp(p_from_third, "inf", 3) == 0) || (std::strncmp(p_from_third, "Inf", 3) == 0)
...
public:
    ...
    basic_node_type parse_block(lexical_token_t lex_type, tag_t tag_type, str_view token, const block_scalar_header & header)
    {
        FK_YAML_ASSERT(lex_type == lexical_token_t::BLOCK_LITERAL_SCALAR || lex_type == lexical_token_t::BLOCK_FOLDED_SCALAR);
        FK_YAML_ASSERT(tag_type != tag_t::SEQUENCE && tag_type != tag_t::MAPPING);
        ...
        do
        {
            ...
            if (token[pos] == '\\')
            {
                ...
                if (token[pos + 1] != '\n')
                {
                    ...
                    {
                        throw parse_error("Unsupported escape sequence is found in a double quoted scalar.", m_line, m_indent);
                    }
    ...
    static std::string normalize_tag_name(const str_view tag, const std::shared_ptr<doc_metainfo_type> & directives)
    {
        ...
        default: {
            ...
            if (tag_end_pos != std::string::npos)
            {
                ...
                {
                    throw invalid_tag("named handle has not been registered.", std::string(tag.begin(), tag.end()).c_str());
                }
...
private:
    ...
    basic_node_type deserialize_document(lexer_type & lexer, lexical_token_t & last_type)
    {
        ...
        case lexical_token_t::SEQUENCE_BLOCK_PREFIX: {
            ...
            parse_context context(lexer.get_lines_processed(), lexer.get_last_token_begin_pos(), context_state_t::BLOCK_SEQUENCE, &root);
        ...
        case lexical_token_t::EXPLICIT_KEY_PREFIX: {
            ...
            apply_directive_set(root);
            apply_node_properties(root);
            parse_context context(lexer.get_lines_processed(), lexer.get_last_token_begin_pos(), context_state_t::BLOCK_MAPPING, &root);
            m_context_stack.emplace_back(std::move(context));
            line = lexer.get_lines_processed();
            indent = lexer.get_last_token_begin_pos();
            break;
    ...
    void deserialize_directives(lexer_type & lexer, lexical_token & last_token)
    {
        ...
        for (;;)
        {
            ...
            case lexical_token_t::YAML_VER_DIRECTIVE:
                ...
                {
                    throw parse_error(
                        "YAML version cannot be specified more than once.", lexer.get_lines_processed(), lexer.get_last_token_begin_pos());
                }
            ...
            case lexical_token_t::TAG_DIRECTIVE: {
                ...
                case 1 /* ! */: {
                    ...
                    {
                        throw parse_error(
                            "Primary handle cannot be specified more than once.", lexer.get_lines_processed(),
                ...
                case 2 /* !! */: {
                    ...
                    {
                        throw parse_error(
                            "Secondary handle cannot be specified more than once.", lexer.get_lines_processed(),
                            lexer.get_last_token_begin_pos());
                    }
                    const str_view tag_prefix = lexer.get_tag_prefix();
                ...
                default /* !<handle>! */: {
                    ...
                    {
                        throw parse_error(
                            "The same named handle cannot be specified more than once.", lexer.get_lines_processed(),
                            lexer.get_last_token_begin_pos());
                    }
                    lacks_end_of_directives_marker = true;
                    break;
            ...
            default:
                ...
                {
                    throw parse_error(
                        "The end of directives marker (---) is missing after directives.", lexer.get_lines_processed(),
                        lexer.get_last_token_begin_pos());
                }
    ...
    void deserialize_node(
        lexer_type & lexer, const lexical_token & first_token, uint32_t first_line, uint32_t first_indent, lexical_token_t & last_type)
    {
        ...
        do
        {
            ...
            case lexical_token_t::EXPLICIT_KEY_PREFIX: {
                ...
                if (token.type == lexical_token_t::SEQUENCE_BLOCK_PREFIX)
                {
                    ...
                    parse_context context(
                        lexer.get_lines_processed(), lexer.get_last_token_begin_pos(), context_state_t::BLOCK_SEQUENCE, p_node);
                    m_context_stack.emplace_back(context);
                ...
                // heap-allocated node will be freed in handling the corresponding KEY_SEPARATOR event
                m_context_stack.emplace_back(line, indent, context_state_t::BLOCK_MAPPING_EXPLICIT_KEY, new basic_node_type());
            ...
            case lexical_token_t::KEY_SEPARATOR: {
                    ...
                    if (token.type == lexical_token_t::SEQUENCE_BLOCK_PREFIX)
                    {
                        ...
                        *mp_current_node = basic_node_type::sequence({basic_node_type()});
                        ...
                        if (has_props)
                        {
                            ...
                            indent = lexer.get_last_token_begin_pos();
                            *mp_current_node = basic_node_type::mapping();
                            m_context_stack.emplace_back(line_after_props, indent, context_state_t::BLOCK_MAPPING, mp_current_node);
                            apply_directive_set(*mp_current_node);
                            apply_node_properties(*mp_current_node);
                        }
                ...
                m_context_stack.emplace_back(old_line, old_indent, context_state_t::BLOCK_MAPPING_EXPLICIT_VALUE, mp_current_node);
            ...
            case lexical_token_t::SEQUENCE_BLOCK_PREFIX: {
                ...
                if (indent == parent_indent)
                {
                    ...
                    // # -> {foo: [null, bar]}
                    // ```
                    pop_to_parent_node(line, indent, [](const parse_context & c) { return c.state == context_state_t::BLOCK_SEQUENCE; });
                }
            ...
            case lexical_token_t::MAPPING_FLOW_BEGIN:
                ...
                case context_state_t::FLOW_MAPPING:
                    ...
                    m_context_stack.emplace_back(line, indent, context_state_t::FLOW_MAPPING_KEY, new basic_node_type(node_type::MAPPING));
                    mp_current_node = m_context_stack.back().p_node;
                    break;
                default: {
                    *mp_current_node = basic_node_type::mapping();
            ...
            case lexical_token_t::BLOCK_FOLDED_SCALAR: {
                ...
                basic_node_type node =
                    scalar_parser_type(line, indent).parse_block(token.type, tag_type, token.str, lexer.get_block_scalar_header());
                apply_directive_set(node);
                apply_node_properties(node);
    ...
    bool deserialize_node_properties(lexer_type & lexer, lexical_token & last_token, uint32_t & line, uint32_t & indent)
    {
        ...
        do
        {
            ...
            case lexical_token_t::ANCHOR_PREFIX:
                ...
                {
                    throw parse_error(
                        "anchor name cannot be specified more than once to the same node.", lexer.get_lines_processed(),
                        lexer.get_last_token_begin_pos());
                }
            ...
            case lexical_token_t::TAG_PREFIX: {
                ...
                {
                    throw parse_error(
                        "tag name cannot be specified more than once to the same node.", lexer.get_lines_processed(),
                        lexer.get_last_token_begin_pos());
                }
    ...
    void add_new_key(basic_node_type && key, const uint32_t line, const uint32_t indent)
    {
        ...
        m_context_stack.emplace_back(key_context.line, key_context.indent, context_state_t::MAPPING_VALUE, mp_current_node);
    }
    ...
    void deserialize_scalar(lexer_type & lexer, basic_node_type && node, uint32_t & indent, uint32_t & line, lexical_token & token)
    {
        token = lexer.get_next_token();
        if (mp_current_node->is_mapping())
        {
            const bool is_key_sep_followed = (token.type == lexical_token_t::KEY_SEPARATOR) && (line == lexer.get_lines_processed());
            if FK_YAML_UNLIKELY (!is_key_sep_followed)
            {
                throw parse_error(
                    "The \":\" mapping value indicator must be followed after a mapping key.", lexer.get_lines_processed(),
                    lexer.get_last_token_begin_pos());
            }
            ...
            if (mp_current_node->is_scalar())
            {
                ...
                else
                {
                    ...
                    *mp_current_node = basic_node_type::mapping();
                    ...
                    {
                        mp_current_node->add_anchor_name(std::string(m_root_anchor_name.begin(), m_root_anchor_name.end()));
                        m_root_anchor_name = {};
                    }
...
inline utf_encode_t detect_encoding_type(const std::array<uint8_t, 4> & bytes, bool & has_bom) noexcept
{
    ...
    if (byte0 == static_cast<uint8_t>(0xEFu) && byte1 == static_cast<uint8_t>(0xBBu) && byte2 == static_cast<uint8_t>(0xBFu))
    {
        has_bom = true;
...
public:
    ...
    str_view get_buffer_view()
    {
        ...
        for (const auto & cr_itr : cr_itrs)
        {
            std::transform(current, cr_itr, std::back_inserter(m_buffer), [](char8_t c) { return static_cast<char>(c); });
            current = std::next(cr_itr);
        }
    ...
    str_view get_buffer_view()
    {
        ...
        while (current != m_end)
        {
            ...
            const auto utf32 = static_cast<char32_t>(
                ((tmp & 0xFF000000u) >> shift_bits[0]) | ((tmp & 0x00FF0000u) >> shift_bits[1]) | ((tmp & 0x0000FF00u) << shift_bits[2])
                | ((tmp & 0x000000FFu) << shift_bits[3]));
...
private:
    ...
    str_view get_buffer_view_utf16()
    {
        ...
        { // m_encode_type == utf_encode_t::UTF_16LE
            shift_bits[1] = 8;
        }
    ...
    str_view get_buffer_view_utf32()
    {
        ...
        { // m_encode_type == utf_encode_t::UTF_32LE
            shift_bits[1] = 8;
            shift_bits[2] = 16;
            shift_bits[3] = 24;
        }
...
{
    static_assert(is_basic_node<BasicNodeType>::value, "iterator_holder class only accepts a basic_node as its template parameter.");
...
class iterator
{
    ...
    using other_iterator_type = typename std::conditional<
        ...
        iterator<typename std::remove_const<ValueType>::type>,
...
public:
    ...
    template<
        ...
        enable_if_t<conjunction<std::is_same<OtherIterator, other_iterator_type>, std::is_const<ValueType>>::value, int> = 0>
    ...
    template<
        typename Iterator,
        enable_if_t<disjunction<std::is_same<Iterator, iterator>, std::is_same<Iterator, other_iterator_type>>::value, int> = 0>
...
{
    static_assert(is_basic_node<BasicNodeType>::value, "map_range_proxy only accepts a basic_node type as its template parameter.");

public:
    /// @brief The type of non-const iterators.
    using iterator = map_iterator_proxy<typename std::conditional<
        std::is_const<BasicNodeType>::value,
        typename BasicNodeType::const_iterator,
...
private:
    ...
    void serialize_node(const BasicNodeType & node, const uint32_t cur_indent, std::string & str)
    {
        ...
        case node_type::STRING: {
            ...
            const node_type type_if_plain = scalar_scanner::scan(str_val.c_str(), str_val.c_str() + str_val.size()); // LCOV_EXCL_LINE
...
template<typename T>
using is_constructible_mapping_type = conjunction<detect::has_key_type<T>, detect::has_mapped_type<T>, detect::has_value_type<T>>;
...
using is_constructible_sequence_type = conjunction<
    negation<is_basic_node<T>>,
    detect::has_iterator<T>,
    detect::is_iterator_traits<typename T::iterator>,
    detect::has_begin_end<T>,
    negation<std::is_same<T, typename BasicNodeType::mapping_type>>,
...
/// @tparam T A target type for detection.
template<typename T>
using is_sequence_container_adapter =
    conjunction<negation<is_basic_node<T>>, detect::has_container_type<T>, detect::has_value_type<T>, negation<detect::has_key_type<T>>>;
...
inline auto from_node(const BasicNodeType & n, T (&array)[N]) -> decltype(n.get_value_inplace(std::declval<T &>()), void())
{
...
inline auto from_node(const BasicNodeType & n, T (&array)[N0][N1]) -> decltype(n.get_value_inplace(std::declval<T &>()), void())
{
    if FK_YAML_UNLIKELY (!n.is_sequence())
    {
        throw type_error("The target node value type is not sequence type.", n.get_type());
...
inline auto from_node(const BasicNodeType & n, T (&array)[N0][N1][N2]) -> decltype(n.get_value_inplace(std::declval<T &>()), void())
{
    if FK_YAML_UNLIKELY (!n.is_sequence())
    {
        throw type_error("The target node value type is not sequence type.", n.get_type());
...
template<typename BasicNodeType, typename T, std::size_t N>
inline auto from_node(const BasicNodeType & n, std::array<T, N> & arr) -> decltype(n.get_value_inplace(std::declval<T &>()), void())
{
    if FK_YAML_UNLIKELY (!n.is_sequence())
    {
        throw type_error("The target node value type is not sequence type.", n.get_type());
...
template<typename BasicNodeType, typename T>
inline auto from_node(const BasicNodeType & n, std::valarray<T> & va) -> decltype(n.get_value_inplace(std::declval<T &>()), void())
{
    if FK_YAML_UNLIKELY (!n.is_sequence())
    {
        throw type_error("The target node value type is not sequence type.", n.get_type());
...
inline auto from_node(const BasicNodeType & n, std::forward_list<T, Alloc> & fl) -> decltype(n.template get_value<T>(), void())
{
    if FK_YAML_UNLIKELY (!n.is_sequence())
    {
...
template<
    typename BasicNodeType,
    typename CompatSeqType,
    enable_if_t<
        conjunction<
            is_basic_node<BasicNodeType>,
            is_constructible_sequence_type<BasicNodeType, CompatSeqType>,
...
template<
    typename BasicNodeType,
    typename SeqContainerAdapter,
    enable_if_t<conjunction<is_basic_node<BasicNodeType>, is_sequence_container_adapter<SeqContainerAdapter>>::value, int> = 0>
...
template<typename BasicNodeType, typename IntType, bool = std::is_same<typename BasicNodeType::integer_type, IntType>::value>
...
template<typename BasicNodeType, typename FloatType, bool = std::is_same<typename BasicNodeType::float_number_type, FloatType>::value>
...
template<
    ...
    typename T,
    enable_if_t<
        conjunction<is_basic_node<BasicNodeType>, std::is_same<typename BasicNodeType::sequence_type, remove_cvref_t<T>>>::value,
        int> = 0>
...
template<
    ...
    enable_if_t<
        conjunction<
            ...
            negation<is_basic_node<CompatSeqType>>,
            detect::has_begin_end<CompatSeqType>,
...
template<
    ...
    typename T,
    enable_if_t<
        conjunction<is_basic_node<BasicNodeType>, std::is_same<typename BasicNodeType::mapping_type, remove_cvref_t<T>>>::value,
        int> = 0>
...
template<
    typename BasicNodeType,
    typename CompatMapType,
    enable_if_t<
        conjunction<
            is_basic_node<BasicNodeType>,
            negation<is_basic_node<CompatMapType>>,
            negation<std::is_same<typename BasicNodeType::mapping_type, remove_cvref_t<CompatMapType>>>,
            detect::has_begin_end<CompatMapType>,
            detect::has_key_type<CompatMapType>,
...
public:
    ...
    static auto from_node(BasicNodeType && n, TargetType & val) noexcept(noexcept(::fkyaml::from_node(std::forward<BasicNodeType>(n), val)))
    ...
    template<typename BasicNodeType, typename TargetType = ValueType>
    static auto to_node(BasicNodeType & n, TargetType && val) noexcept(noexcept(::fkyaml::to_node(n, std::forward<TargetType>(val))))
...
template<
    typename Key,
    typename Value,
    typename IgnoredCompare = std::less<Key>,
...
public:
    ...
    template<typename KeyType, detail::enable_if_t<detail::is_usable_as_key_type<key_compare, key_type, KeyType>::value, int> = 0>
...
public:
    ...
    template<
        ...
        typename U = detail::remove_cvref_t<CompatibleType>,
        detail::enable_if_t<
            detail::conjunction<
    ...
    /// @sa https://fktn-k.github.io/fkYAML/api/basic_node/constructor/
    template<typename NodeRefStorageType, detail::enable_if_t<detail::is_node_ref_storage<NodeRefStorageType>::value, int> = 0>
    ...
    {
        bool is_mapping = std::all_of(init.begin(), init.end(), [](const detail::node_ref_storage<basic_node> & node_ref) {
...
public:
    ...
    {
        return deserializer_type().deserialize(detail::input_adapter(std::forward<ItrType>(begin), std::forward<ItrType>(end)));
    }
    ...
    {
        return deserializer_type().deserialize_docs(detail::input_adapter(std::forward<ItrType>(begin), std::forward<ItrType>(end)));
    }
...
public:
    ...
    template<
        typename KeyType,
        detail::enable_if_t<
            detail::conjunction<detail::negation<detail::is_basic_node<KeyType>>, detail::is_node_compatible_type<basic_node, KeyType>>::
                value,
            int> = 0>
    ...
    bool operator==(const basic_node & rhs) const noexcept
    {
        ...
        case detail::node_attr_bits::int_bit:
            ...
            break;
        case detail::node_attr_bits::float_bit:
            ret = (std::abs(lhs.m_value.float_val - act_rhs.m_value.float_val) < std::numeric_limits<float_number_type>::epsilon());
            break;
        case detail::node_attr_bits::string_bit:
...
public:
    ...
    template<
        typename T,
        typename ValueType = detail::remove_cv_t<T>,
        detail::enable_if_t<
            detail::conjunction<std::is_default_constructible<ValueType>, detail::negation<std::is_pointer<T>>>::value,
            int> = 0>
    T get_value() const
        noexcept(noexcept(std::declval<const basic_node &>().template get_value_impl<ValueType>(std::declval<ValueType &>())))
    {
        // emit a compile error if T is either a reference, pointer or C-style array type.
        static_assert(
            !std::is_reference<T>::value, "get_value() cannot be called with reference types. "
                                          "You might want to call one of as_seq(), as_map(), as_bool(), as_int(), as_float() or as_str().");
        static_assert(
            !std::is_array<T>::value, "get_value() cannot be called with C-style array types. You might want to call get_value_inplace().");
    ...
    template<
        ...
        typename U,
        detail::enable_if_t<
            detail::conjunction<std::is_constructible<T, U>, std::is_default_constructible<T>, detail::negation<std::is_pointer<T>>>::value,
            int> = 0>
    T get_value_or(U && default_value) const noexcept
    {
        static_assert(
            !std::is_reference<T>::value, "get_value_or() cannot be called with reference types. "
                                          "You might want to call one of as_seq(), as_map(), as_bool(), as_int(), as_float() or as_str().");
        static_assert(
    ...
    template<
        ...
        detail::enable_if_t<
            detail::conjunction<std::is_reference<ReferenceType>, std::is_const<detail::remove_reference_t<ReferenceType>>>::value,
            int> = 0>
    FK_YAML_DEPRECATED("Since 0.4.3; Use one of as_seq(), as_map(), as_bool(), as_int(), as_float() or as_str()")
...
private:
    ...
    template<typename ValueType, detail::enable_if_t<detail::negation<detail::is_basic_node<ValueType>>::value, int> = 0>
...
inline std::ostream & operator<<(
    std::ostream & os,
    const basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType, ConverterType> & n)
{
    os << basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType, ConverterType>::serialize(n);
    return os;
}
...
inline std::istream & operator>>(
    std::istream & is, basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType, ConverterType> & n)
{
    n = basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType, ConverterType>::deserialize(is);
    return is;
}
...
namespace std
{
    ...
    template<
        ...
        template<typename, typename = void> class ConverterType>
    // NOLINTNEXTLINE(cert-dcl58-cpp)
    struct hash<fkyaml::basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType, ConverterType>>
    {
        using node_t = fkyaml::basic_node<SequenceType, MappingType, BooleanType, IntegerType, FloatNumberType, StringType, ConverterType>;
